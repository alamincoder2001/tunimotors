					...
					<tbody>
						<template v-for="purchase in purchases">
							<tr>
								...
								<td>{{ purchase.purchaseDetails[0].Product_Name || 'unknown' }}</td>
								<td style="text-align:right;">{{ purchase.purchaseDetails[0].PurchaseDetails_Rate || 'unknown'  }}</td>
								<td style="text-align:center;">{{ purchase.purchaseDetails[0].PurchaseDetails_TotalQuantity || 'unknown'  }}</td>
								<td style="text-align:right;">{{ purchase.purchaseDetails[0].PurchaseDetails_TotalAmount || 'unknown'  }}</td>
							...
							<tr v-if="purchase.purchaseDetails.length > 0" v-for="(product, sl) in purchase.purchaseDetails.slice(1)">
								<td colspan="3" v-bind:rowspan="purchase.purchaseDetails.length - 1" v-if="sl == 0"></td>
								<td>{{ product.Product_Name || 'unknown' }}</td>
								<td style="text-align:right;">{{ product.PurchaseDetails_Rate || 'unknown' }}</td>
								<td style="text-align:center;">{{ product.PurchaseDetails_TotalQuantity || 'unknown'}}</td>
								<td style="text-align:right;">{{ product.PurchaseDetails_TotalAmount || 'unknown' }}</td>
								<td></td>
							</tr>
							<tr style="font-weight:bold;">
								<td colspan="3"></td>
								<td colspan="">
									<span style="text-decoration: underline;">Engine No</span>
									<ul v-for="item in purchase.purchaseDetails">
										<li v-for="(item1, index)  in item.engine">{{ index+1 }}: {{ item1.EngineNo }}</li>
									</ul>
								</td>
								<td style="text-align:right;" colspan="">
									<span style="text-decoration: underline;">Chassis No</span>
									<ul v-for="item in purchase.purchaseDetails">
										<li v-for="(item1, index) in item.engine">{{ index+1 }}: {{ item1.chassisNo }}</li>
									</ul>
								</td>
	...
	new Vue({
		...
		data(){
			return {
				...
				searchTypesForDetails: ['quantity', 'category', 'chassis', 'engine'],
				chassis: "",
				engine: ""
			}
		},
		methods: {
			...
			async print(){
				...
				if(this.selectedProduct != null && this.selectedProduct.Product_SlNo != '' && this.searchType == 'quantity'){
					productText = `<strong>Product: </strong> ${this.selectedProduct.Product_Name || null }`;
				}
