			...
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				<div class="form-group">
					...
					<select class="form-control" v-model="searchType" @change="onChangeSearchType">
						...
						<option value="quantity">By Discount</option>
						<option value="user">By User</option>
						<option value="color">By Color</option>
						<option value="engine">By Engine</option>
						<option value="chassis">By Chassis</option>
					</select>
				</div>
				...
				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'color' && colors.length > 0 ? '' : 'none'}">
					<label>Color</label>
					<v-select v-bind:options="colors" v-model="selectedColor" label="color_name"></v-select>
				</div>

				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'chassis' ? '' : 'none'}">
					<label> Chassis </label>
					<input type="text" v-model="chassis" class="form-control" label="Model">
				</div>

				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'engine' ? '' : 'none'}">
					<label> Engine </label>
					<input type="text" v-model="engine" class="form-control" label="Model">
				</div>
					...
					<tbody>
						<template v-for="sale in sales">
							<tr>
								...
								<td style="text-align:right;">{{ sale.saleDetails[0].EngineNo }}</td>
								<td style="text-align:right;">{{ sale.saleDetails[0].chassisNo }}</td>
							...
							<tr v-for="(product, sl) in sale.saleDetails.slice(1)">
								...
								<td style="text-align:right;">{{ product.EngineNo }}</td>
								<td style="text-align:right;">{{ product.chassisNo }}</td>
								<td></td>
							...
							<tr style="font-weight:bold;">
								<td colspan="8"></td>
								<td colspan="2">
									Offer: {{ sale.saleDetails[0].Discount_amount }} <br>
									Special: {{ sale.special_discount }} <br>
									Agent: {{ sale.agent_discount }} <br>
									Showroom: {{ sale.showroom_discount }} <br>
									Reference: {{ sale.showroom_discount_reference }} <br>
									Head Office: {{ sale.head_office_discount }} <br>
									Reference: {{ sale.head_office_discount_reference }} <br>
								</td>
					...
					<table class="record-table" v-if="selectedProduct != null">
						<thead>
							<tr>
								...
								<th>Customer Name</th>
								<th>Product Name</th>
								<th>Showroom Discount</th>
								<th>Showroom Feferance</th>
								<th>Special Discount</th>
								<th>HeadOffice Discount</th>
								<th>HeadOffice Feferance</th>
						...
						<tbody>
							<tr v-for="sale in sales">
								...
								<td>{{ sale.SaleMaster_SaleDate }}</td>
								<td>{{ sale.Customer_Name }}</td>
								<td>{{ sale.Product_Name}} - {{sale.EngineNo }} - {{sale.chassisNo }}</td>
								<td>{{ sale.showroom_discount }}</td>
								<td>{{ sale.showroom_discount_reference }}</td>
								<td>{{ sale.special_discount }}</td>
								<td>{{ sale.head_office_discount }}</td>
								<td>{{ sale.head_office_discount_reference }}</td>
								<td>{{ sale.agent_discount }}</td>
						...
						<tfoot>
							<tr style="font-weight:bold;">
								...
								<td style="text-align:right;">{{ sales.reduce((prev, curr) => { return prev + parseFloat(curr.showroom_discount)}, 0) }}</td>
								<td></td>
								<td style="text-align:right;">{{ sales.reduce((prev, curr) => { return prev + parseFloat(curr.special_discount)}, 0) }}</td>
								<td style="text-align:right;">{{ sales.reduce((prev, curr) => { return prev + parseFloat(curr.head_office_discount)}, 0) }}</td>
								<td></td>
					...
					<table class="record-table" v-if="selectedProduct == null">
						<thead>
							<tr>
								...
								<th>Showroom Discount</th>
								<th>Special Discount</th>
								<th>Head Office Discount</th>
								<th>Agent Discount Discount</th>
								<th>Quantity</th>
							</tr>
						</thead>
						<tbody>
							<template v-for="sale in sales">
								...
								<tr v-for="product in sale.products">
									...
									<td style="text-align:right;">{{ product.head_office_discount }}</td>
	...
	new Vue({
		...
		data() {
			return {
				...
				colors: [],
				selectedColor: null,
				searchTypesForRecord: ['', 'user', 'customer', 'employee', 'chassis', 'engine'],
				searchTypesForDetails: ['quantity', 'category', 'color'],
				chassis: "",
				engine: ""
			}
		},
