...
<?php
    ...
    public function deleteTumiMotors()
    {
        $validEngin = ['G3J3E0584933', 'G3J3E0584404', 'E31EE0164831', 'E31EE0164905', 'G3N5E0239334', 'E31EE0160052', 'E31EE0165734', 'G3L5E0239700', 'G3L5E0239696', 'G3L5E0239388', 'G3L5E0239695', 'G3L5E0239649', 'G3L5E0283581', 'G3L5E0292089', 'G3L5E0291976', 'G3L5E0292172', 'G3L5E0282791', 'G3L5E0281760', 'G3L5E0281906', 'G3L5E0281850', 'G3L5E0291386', 'G3L5E0291397', 'G3L5E0286579', 'G3L5E0286599', 'G3L5E0284926', 'G3L5E0284507', 'G3L5E0271086', 'G3L5E0279995', 'G3L5E0279983', 'G3L5E0278476'];

        $alldata = $this->db->query("SELECT * FROM `tbl_engine` WHERE `status` = 'a' and `salesID` = 0 and `Branch_id` = 1")->result();
        ...
        for ($i = 0; $i < count($alldata); $i++) {
            foreach ($alldata as $key2 => $value2) {
                if ($validEngin[$i] == $value2->EngineNo) {
                    unset($alldata[$key2]);
                }
            }
        }

        foreach ($alldata as $key => $engin) {
            // $purchaseMaster = $this->db->query("SELECT * FROM `tbl_purchasemaster` WHERE `PurchaseMaster_SlNo` = ?", $engin->PurchaseMaster_SlNo)->row();

            $this->db->query("update tbl_purchasemaster
                set PurchaseMaster_TotalAmount = PurchaseMaster_TotalAmount - ?,
                PurchaseMaster_SubTotalAmount = PurchaseMaster_SubTotalAmount - ?,
                PurchaseMaster_DueAmount = PurchaseMaster_DueAmount - ?
                where PurchaseMaster_SlNo = ?                
            ", [$engin->Purchase_Rate, $engin->Purchase_Rate, $engin->Purchase_Rate, $engin->PurchaseMaster_SlNo]);

            // $purchaseDetails = $this->db->query("SELECT * FROM `tbl_purchasedetails` WHERE `PurchaseDetails_SlNo` = ?", $engin->PurchaseDetails_SlNo)->row();

            $this->db->query("update tbl_purchasedetails
            set PurchaseDetails_TotalQuantity = PurchaseDetails_TotalQuantity - 1,
            PurchaseDetails_TotalAmount = PurchaseDetails_TotalAmount - ?
            where PurchaseDetails_SlNo = ?                
        ", [$engin->Purchase_Rate, $engin->PurchaseDetails_SlNo]);

            $this->db->query("update tbl_engine set status = 'd' where engine_id = ?", $engin->engine_id);
        }
