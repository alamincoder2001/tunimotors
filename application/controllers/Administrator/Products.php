...
class Products extends CI_Controller
{
    ...
    public function getProducts()
    {
        ...
        if (isset($data->unitId) && $data->unitId != '') {
            $clauses .= " and p.Unit_ID = '$data->unitId'";
        }
    ...
    {
        $result = $this->db->query("SELECT * from tbl_engine where status= 'a'")->result();
        echo json_encode($result);
    }
    public function getEngineNO()
    {
        $inputs = json_decode($this->input->raw_input_stream);
        $enginDetails = $this->db->query("
            select 
            *,
            CONCAT(EngineNo,' - ',chassisNo) as display_text
        from tbl_engine where productId = ? and Branch_id = ? and status= 'a'
        ", [$inputs->productId, $this->session->userdata("BRANCHid")])->result();
        echo json_encode($enginDetails);
    }
    ...
    public function getStockDetails()
    {
        ...
        $data = json_decode($this->input->raw_input_stream);
        $stockDetails = $this->db->query("select * from tbl_engine where status = 'a' and productId = ? and Branch_id = ?", [$data->product_id, $this->session->userdata('BRANCHid')])->result();

        $res['stockDetails'] = $stockDetails;
    ...
    public function getTotalStock()
    {
        ...
        if (isset($data->date) && $data->date != null) {
            $clauses .= " and p.AddTime BETWEEN '2000-01-01' and '$data->date'";
        }
    ...
    public function getSoldStock()
    {
        ...
        // } 
        $stock = [];
        foreach ($data->cartItemsDetails as $key => $item) {
            $saleDetail = $this->db->query("
                select * from tbl_saledetails 
                    where Engin_Id = ? 
                    and SaleDetails_BranchId = ?
                    and Status = 'a'
            ", [$item->engine_id, $branchId])->result();
            array_merge($stock, $saleDetail);
        }
