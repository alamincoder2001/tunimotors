...
class Purchase extends CI_Controller
{
    ...
    public function getPurchases()
    {
        ...
        if (isset($data->purchaseId) && $data->purchaseId != null) {
            ...
            $purchaseDetails = $this->db->query("
                ...
                where pd.PurchaseMaster_IDNo = '$data->purchaseId' and pd.Status = 'a'
            ")->result();

            foreach ($purchaseDetails as $detail) {
                $detail->engines = $this->db->query("
                    select * from tbl_engine 
                    where PurchaseDetails_SlNo = ? and Branch_id = ?
                    and status != 'd'
                ", [$detail->PurchaseDetails_SlNo, $branchId])->result();
            }
    ...
    public function addPurchase()
    {
        ...
        try {
            ...
            foreach ($data->cartProducts as $product) {
                ...
                $purchaseDetailsID = $this->db->insert_id();
                ...
                foreach ($product->cartItemsDetails as $items) {
                    $purchaseDetails = array(
                        'PurchaseMaster_SlNo'  => $purchaseId,
                        'PurchaseDetails_SlNo' => $purchaseDetailsID,
                        'purchaseRate'         => $product->purchaseRate,
                        'productId'            => $product->productId,
                        'Purchase_Rate'        => $product->purchaseRate,
                        'chassisNo'            => $items->chassisNo,
                        'EngineNo'             => $items->EngineNo,
                        'status'               => 'a',
                        'added_by'             => $this->session->userdata("FullName"),
                        'addDate'              => date('Y-m-d H:i:s'),
                        'Branch_id'            => $this->session->userdata('BRANCHid')
                    );
                    $this->db->insert('tbl_engine', $purchaseDetails);
                }
            }
    ...
    public function updatePurchase()
    {
        ...
        try {
            ...
            $oldPurchaseDetails = $this->db->query("select * from tbl_purchasedetails where PurchaseMaster_IDNo = ? and Status = 'a'", $purchaseId)->result();

            $this->db->query("update tbl_purchasedetails set Status = 'd' where PurchaseMaster_IDNo = ?", $purchaseId);

            foreach ($oldPurchaseDetails as $product) {
                $this->db->query("update tbl_engine set status = 'd' where PurchaseDetails_SlNo = ? ", $product->PurchaseDetails_SlNo);
    ...
    public function updateEngine()
    {
        ...
        try {
            ...
            $numExist = $this->db->query("
                select * from tbl_engine
                where status != 'd' and engine_id != '$data->engine_id'
                and (EngineNo = '$data->EngineNo' or chassisNo = '$data->chassisNo')
            ")->num_rows();

            if ($numExist) {
                $res = ['success' => false, 'message' => 'Duplicate Value Exist!'];
                echo json_encode($res);
                return 0;
            }
            ...
            $this->db->query("
                update tbl_engine
                set EngineNo = ?, chassisNo = ?
                where engine_id = ?
                and branch_id = ? and status != 'd'
            ", [$data->EngineNo, $data->chassisNo, $data->engine_id, $this->session->userdata('BRANCHid')]);
    ...
    public function deleteEngine()
    {
        ...
        try {
            ...
            $numExist = $this->db->query("
                select * from tbl_saledetails
                where status != 'd'
                and Engin_Id = '$data->engine_id'
            ")->num_rows();

            if ($numExist) {
                $res = ['success' => false, 'message' => 'This product already sold!'];
                echo json_encode($res);
                return 0;
            }
            $engine = $this->db->query("
                ...
                where engine_id = ?
                and Branch_id = ? and status != 'd'
            ", [$data->engine_id, $this->session->userdata('BRANCHid')])->row();

            $this->db->query("
                update tbl_purchasedetails 
                set PurchaseDetails_TotalQuantity = PurchaseDetails_TotalQuantity - 1,
                PurchaseDetails_TotalAmount = PurchaseDetails_TotalAmount - PurchaseDetails_Rate
                where PurchaseDetails_SlNo = ?
            ", $engine->PurchaseDetails_SlNo);

            $purchaseDetail = $this->db->query("
                select * from tbl_purchasedetails
                where PurchaseDetails_SlNo = ?
            ", $engine->PurchaseDetails_SlNo)->row();

            $this->db->query("
                update tbl_purchasemaster
                set PurchaseMaster_TotalAmount = PurchaseMaster_TotalAmount - '$purchaseDetail->PurchaseDetails_Rate',
                PurchaseMaster_SubTotalAmount = PurchaseMaster_SubTotalAmount - '$purchaseDetail->PurchaseDetails_Rate',
                PurchaseMaster_DueAmount = PurchaseMaster_TotalAmount - PurchaseMaster_PaidAmount
                where PurchaseMaster_SlNo = ?
            ", $engine->PurchaseMaster_SlNo);

            $this->db->query("
                update tbl_currentinventory
                set purchase_quantity = purchase_quantity - 1
                where product_id = ? and branch_id = ?
            ", [$engine->productId, $this->session->userdata('BRANCHid')]);
            ...
            $this->db->query("
                ...
                where engine_id = ?
                and branch_id = ? and status != 'd'
            ", [$data->engine_id, $this->session->userdata('BRANCHid')]);

            $res = ['success' => true, 'message' => 'Delete Success'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
        echo json_encode($res);
    ...
    public function getPurchaseRecord()
    {
        ...
        foreach ($purchases as $purchase) {
            ...
            $purchaseDetails = array_map(function ($detail) {
                $detail->engine = $this->db->query("
                    select * from tbl_engine 
                    where PurchaseDetails_SlNo = ? 
                    and Branch_id = ?
                    ", [$detail->PurchaseDetails_SlNo, $this->session->userdata("BRANCHid")])->result();
                return $detail;
            }, $purchaseDetails);

            $purchase->purchaseDetails = $purchaseDetails;

            // foreach($purchase->purchaseDetails as $engine){
            //     $purchase->purchaseDetails->engine = $this->db->query("
            //         select 
            //             e.*
            //         from tbl_engine e
            //         where e.PurchaseDetails_SlNo = ?

            //     ", $engine->PurchaseDetails_SlNo)->result();
            // }
        }
    ...
    public function getPurchaseDetails()
    {
        ...
        if (isset($data->chassis) && $data->chassis != '') {
            $clauses .= " and e.chassisNo = '$data->chassis'";
        }
        if (isset($data->engine) && $data->engine != '') {
            $clauses .= " and e.EngineNo = '$data->engine'";
        }
        if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
            $clauses .= " and pm.PurchaseMaster_OrderDate between '$data->dateFrom' and '$data->dateTo'";
        }
        ...
        $saleDetails = $this->db->query("
            ...
            join tbl_purchasemaster pm on pm.PurchaseMaster_SlNo = pd.PurchaseMaster_IDNo
            join tbl_supplier s on s.Supplier_SlNo = pm.Supplier_SlNo
            join tbl_engine e on e.PurchaseDetails_SlNo = pd.PurchaseDetails_SlNo
            where pd.Status != 'd'
            $clauses
        ")->result();
    ...
    public function  deletePurchase()
    {
        ...
        try {
            ...
            $this->db->set('status', 'd')->where('PurchaseMaster_SlNo', $data->purchaseId)->update('tbl_engine');
