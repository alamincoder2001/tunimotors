...
class Sales extends CI_Controller {
    ...
    public function addSales(){
        ...
        try{
            ...
            $totalDiscount = $data->sales->discount + $data->sales->showroom_discount + $data->sales->special_discount + $data->sales->head_office_discount + $data->sales->agent_discount;
            ...
            $sales = array(
                ...
                'due_reminder_date' => $data->sales->due_reminder_date,
                'SaleMaster_SaleType' => $data->sales->salesType,
                'SaleMaster_TotalSaleAmount' => $data->sales->total,
                'SaleMaster_TotalDiscountAmount' => $totalDiscount,
                'showroom_discount' => $data->sales->showroom_discount,
                'showroom_discount_reference' => $data->sales->showroom_discount_reference,
                'special_discount' => $data->sales->special_discount,
                'head_office_discount' => $data->sales->head_office_discount,
                'head_office_discount_reference' => $data->sales->head_office_discount_reference,
                'agent_discount' => $data->sales->agent_discount,
            ...
            foreach($data->cart as $cartProduct){
                $saleDetails = array(
                    ...
                    'Engin_Id' => $cartProduct->engin->engine_id,
                    'Status' => 'a',
                    'AddBy' => $this->session->userdata("FullName"),
                    'AddTime' => date('Y-m-d H:i:s'),
                ...
                $this->db->query("
                    ...
                    where engine_id = ?
                    and branch_id = ?
                ", [$detailId, $cartProduct->salesRate, $cartProduct->engin->engine_id, $this->session->userdata('BRANCHid')]);
    ...
    public function getSaleDetails(){
        ...
        if(isset($data->colorId) && $data->colorId != '') {
            $clauses .= " and p.color = '$data->colorId'";
        }
        ...
        $saleDetails = $this->db->query("
            select 
                ...
                sm.showroom_discount,
                sm.special_discount,
                sm.head_office_discount,
                sm.agent_discount,
    ...
    public function getDiscount(){
        ...
        $saleDetails = $this->db->query("
            select 
                ...
                sm.agent_discount,
                sm.SaleMaster_TotalDiscountAmount,
                sm.showroom_discount_reference,
                sm.head_office_discount_reference,
                c.Customer_Code,
                c.Customer_Name
            from tbl_saledetails sd
            join tbl_product p on p.Product_SlNo = sd.Product_IDNo
    ...
    public function getSalesRecord(){
        ...
        if(isset($data->engine) && $data->engine != ''){
            $clausesWith .= " and en.EngineNo = '$data->engine'";
        }

        if(isset($data->chassis) && $data->chassis != ''){
            $clausesWith .= " and en.chassisNo = '$data->chassis'";
        }
        ...
        $sales = $this->db->query("
            ...
            left join tbl_saledetails sd on sd.SaleMaster_IDNo = sm.SaleMaster_SlNo
            left join tbl_engine en on en.engine_id  = sd.Engin_Id
    ...
    public function getTotalDiscount(){
        ...
        SELECT 
            ...
            sum(sm.special_discount) as special_discount_total,
            sum(sm.head_office_discount) as head_office_discount,
            sum(sm.showroom_discount) as showroom_total_discount,
            sum(sm.agent_discount) as agent_total_discount
        FROM tbl_salesmaster sm
        JOIN tbl_saledetails as sd on sd.SaleMaster_IDNo = sm.SaleMaster_SlNo
        
        WHERE sm.AddTime BETWEEN '2000-01-01' and '$data->date'
        ")->row();
        $res['discount'] = $discount;
        
        $discountIn = $this->db->query("
        SELECT sum(In_Amount) as total_in 
        FROM tbl_cashtransaction 
        WHERE Acc_SlID = 4
        and AddTime BETWEEN '2000-01-01' and ' $data->date'
        ")->row();
        $res['discountIn'] = $discountIn;
        $ACIclime = $this->db->query("
        SELECT sum(In_Amount) as total_in, SUM(Out_Amount) as total_out
        FROM tbl_cashtransaction 
        WHERE Acc_SlID = 5
        and AddTime BETWEEN '2000-01-01' and '$data->date'
        ")->row();
        $res['ACIclime'] = $ACIclime;
    ...
    public function getSales(){
        ...
        $clauses1 = "";
        if(isset($data->chassis) && $data->chassis != ''){
            $clauses1 .= " and e.chassisNo = '$data->chassis'";
        }
        if(isset($data->engine) && $data->engine != ''){
            $clauses1 .= " and e.EngineNo = '$data->engine'";
        }
        if((isset($data->chassis) || isset($data->engine)) && ($data->chassis != '' || $data->engine != '')) {
            $saleDetails = $this->db->query("
                select 
                    sd.*,
                    e.*,
                ...
                join tbl_product p on p.Product_SlNo = sd.Product_IDNo
                join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
                join tbl_engine e on e.engine_id = sd.Engin_Id
                join tbl_unit u on u.Unit_SlNo = p.Unit_ID
                where sd.SaleDetails_BranchId = ? $clauses1
            ", $branchId)->result();
        ...
        if(isset($data->salesId) && $data->salesId != 0 && $data->salesId != '') {
            ...
            $saleDetails = $this->db->query("
                ...
                join tbl_engine e on e.engine_id = sd.Engin_Id
                join tbl_unit u on u.Unit_SlNo = p.Unit_ID
                where sd.SaleMaster_IDNo = ? and sd.Status = 'a' 
            ", $data->salesId)->result();
        ...
        $sales = $this->db->query("
            ...
            and sm.Status = 'a'
            $clauses
            order by sm.SaleMaster_SlNo desc")->result();
        $res['sales'] = $sales;
    ...
    public function updateSales(){
        ...
        try{
            ...
            $currentSaleDetails = $this->db->query("select * from tbl_saledetails where SaleMaster_IDNo = ? and Status = 'a' ", $salesId)->result();
            $this->db->query("update tbl_saledetails set Status = 'd' where SaleMaster_IDNo = ?", $salesId);
           
            foreach($currentSaleDetails as $product){
                $this->db->query("
                    update tbl_engine 
                    set status = 'a', salesID = 0, salesPrice = 0 
                    where engine_id = ?
                    and branch_id = ?
                ", [$product->Engin_Id, $this->session->userdata('BRANCHid')]);
            ...
            foreach($data->cart as $cartProduct){
                ...
                $this->db->query("
                    ...
                    where engine_id = ?
                    and branch_id = ?
                ", [$salesDetailId, $cartProduct->salesRate, $cartProduct->engin->engine_id, $this->session->userdata('BRANCHid')]);
    ...
    public function  deleteSales(){
        ...
        try{
            ...
            $saleDetails = $this->db->select('Product_IDNo, SaleDetails_TotalQuantity, Engin_Id')->where('SaleMaster_IDNo', $saleId)->where('Status', 'a')->get('tbl_saledetails')->result();
            ...
            foreach ($saleDetails as $detail){
                $this->db->query("
                    ...
                    where engine_id = ?
                    and branch_id = ?
                ", [$detail->Engin_Id, $this->session->userdata('BRANCHid')]);
...
//    }
    ...
    public function dueReminder() {
        $data['title'] = "Due Reminder";
        $data['content'] = $this->load->view('Administrator/sales/due_reminder', $data, true);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getReminder()
    {
        ...
        select 
            ...
            (select ifnull(sum(cp.CPayment_amount), 0.00) 
                ...
                and cp.CPayment_status = 'a') as CustomerPayment,
            (select sm.SaleMaster_DueAmount - CustomerPayment) as DueReminderAmount
        from tbl_salesmaster sm
        left join tbl_customer c on c.Customer_SlNo = sm.SalseCustomer_IDNo
        where sm.due_reminder_date between '$data->reminderDate' - interval 7 day 
        ...
        $res['beforeReminders'] = $afterReminders;
        ...
        from tbl_salesmaster sm
        left join tbl_customer c on c.Customer_SlNo = sm.SalseCustomer_IDNo
        where sm.due_reminder_date between '$data->reminderDate' and '$data->reminderDate' + interval 7 day  
        and sm.Status = 'a'
        and sm.SaleMaster_DueAmount != 0
        and sm.SaleMaster_branchid = ?
        ", $this->session->userdata('BRANCHid'))->result();

        $res['afterReminders'] = $afterReminders;
